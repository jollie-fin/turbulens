...
//                    fprintf(stderr,"%d %d\n", mouseX, mouseY);
        ...
        Uint8 *src2 = (Uint8 *) video[noold]->pixels;
        ...
        for (int y = 0; y < HEIGHT; y++)
            for (int x = 0; x < WIDTH; x++)
                ...
                {
                    valeur = abs(int(src1[x*4+y*WIDTH*4+xy]) - int(src2[x*4+y*WIDTH*4+xy]));
                    if (valeur>50)
                    {
                    X += x*valeur;
                    Y += y*valeur;
        ...
        if (total > 255)
        {
            ...
            {
            dest[xx*4+yy*WIDTH*4] = 255;
            dest[xx*4+yy*WIDTH*4+1] = 255;
...
{
/*        float Wb = float(mouseX) / 640.;
        float Wr = float(mouseX) / 480.;*/
        no_frame = (no_frame+1)%BUFFER;
        static int i = 0;
        float Y1,Y2,U,V,R,G,B,a,b,c,d;
...
//        dev_name = "/dev/video";
        ...
        for (int i = 0; i < BUFFER; i++)
                video[i] = SDL_CreateRGBSurface(SDL_SWSURFACE, WIDTH, HEIGHT, 32, 0, 0, 0, 0);
